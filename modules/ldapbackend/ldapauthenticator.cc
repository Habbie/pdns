...
bool LdapGssapiAuthenticator::authenticate( LDAP *conn )
{
	...
	if ( code == -1 ) {
		...
		L<<Logger::Debug << "No TGT found, trying to acquire a new one" << std::endl;
		code = updateTgt();

		if ( attemptAuth( conn ) != 0 ) {
			L<<Logger::Error << "Failed to acquire a TGT" << std::endl;
			return false;
		}
	}
...
int LdapGssapiAuthenticator::attemptAuth( LDAP *conn )
{
	...
	L<<Logger::Debug << "ldap_sasl_interactive_bind_s returned " << rc << std::endl;
...
int LdapGssapiAuthenticator::updateTgt()
{
	...
	if ( ( code = krb5_init_context( &context ) ) != 0 ) {
		L<<Logger::Error << "Failed to init krb5 context" << std::endl;
		return code;
	}
	...
	if ( code != 0 ) {
		L<<Logger::Error << "krb5 error: " << std::string( krb5_get_error_message( context, code ) ) << std::endl;
		return code;
	}
