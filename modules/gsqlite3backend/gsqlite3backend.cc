...
public:
  ...
  void declareArguments(const std::string& suffix = "") override
  {
    ...
    declare(suffix, "get-deltas-for-domain-query", "", "SELECT domain_id, fromserial, toserial, name, type, content, ttl, added FROM deltas WHERE domain_id=:domain_id ORDER BY fromserial, toserial, type!='SOA', added");
  }
