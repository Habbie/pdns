...
static int checkZone(DNSSECKeeper &dk, UeberBackend &B, const ZoneName& zone, const vector<DNSResourceRecord>* suppliedrecords=nullptr) // NOLINT(readability-function-cognitive-complexity,readability-identifier-length)
{
  ...
  for( const auto &rr : records ) {
    ...
    if( ! ds_ns && rr.qtype.getCode() == QType::DS && rr.qname != DNSName(zone) ) {
...
static int increaseSerial(const ZoneName& zone, DNSSECKeeper &dk)
{
  ...
  if (!sd.db->replaceRRSet(sd.domain_id, DNSName(zone), rr.qtype, rrs)) {
  ...
  if (sd.db->doesDNSSEC()) {
    ...
    if(haveNSEC3) {
      if(!narrow)
        ordername=DNSName(toBase32Hex(hashQNameWithSalt(ns3pr, DNSName(zone))));
    } else
      ordername=DNSName("");
    if(g_verbose)
...
static int editZone(const ZoneName &zone, const PDNSColors& col) {
 ...
 editMore:;
  ...
  if (!changed.empty()) {
    if (changed.find({DNSName(zone), QType::SOA}) == changed.end()) {
     reAsk3:;
      ...
      switch(c) {
        case 'y':
          {
            DNSRecord oldSoaDR = grouped[{DNSName(zone), QType::SOA}].at(0); // there should be only one SOA record, so we can use .at(0);
            ostringstream str;
...
static int loadZone(const ZoneName& zone, const string& fname) {
  ...
  bool haveSOA = false;
  while(zpt.get(rr)) {
    if(!rr.qname.isPartOf(zone) && rr.qname!=DNSName(zone)) {
...
static int addOrReplaceRecord(bool isAdd, const vector<string>& cmds) {
  ...
  if (cmds.at(2) == "@")
    ...
    name = DNSName(cmds.at(2)) + DNSName(zone);
...
static int deleteRRSet(const std::string& zone_, const std::string& name_, const std::string& type_)
{
  ...
  if(name_=="@")
    ...
    name=DNSName(name_)+DNSName(zone);
...
{
  DNSResourceRecord rr;
  rr.qname=DNSName("blah")+DNSName(zone);
  rr.qtype=QType::A;
  rr.ttl=3600;
  rr.auth=true;
  rr.qclass = QClass::IN;
  ...
  for(unsigned int n=0; n < 100000; ++n) {
    ...
    rr.qname=DNSName(tmp)+DNSName(zone);
    DNSZoneRecord dzr;
    dzr.dr=DNSRecord(rr);
    if(csp.submit(dzr))
...
static bool showZone(DNSSECKeeper& dnsseckeeper, const ZoneName& zone, bool exportDS = false) // NOLINT(readability-function-cognitive-complexity)
{
  ...
  if (dnsseckeeper.isPresigned(zone)) {
    ...
    for(const auto& key : keys) {
      ...
      if (g_verbose) {
        cout<<prefix<<zone.toString()<<" IN DS "<<makeDSFromDNSKey(DNSName(zone), key, DNSSECKeeper::DIGEST_SHA1).getZoneRepresentation() << " ; ( SHA1 digest )" << endl;
      }
      cout<<prefix<<zone.toString()<<" IN DS "<<makeDSFromDNSKey(DNSName(zone), key, DNSSECKeeper::DIGEST_SHA256).getZoneRepresentation() << " ; ( SHA256 digest )" << endl;
      if (g_verbose) {
        try {
          string output=makeDSFromDNSKey(DNSName(zone), key, DNSSECKeeper::DIGEST_GOST).getZoneRepresentation();
      ...
      try {
        string output=makeDSFromDNSKey(DNSName(zone), key, DNSSECKeeper::DIGEST_SHA384).getZoneRepresentation();
...
static int testSchema(DNSSECKeeper& dk, const ZoneName& zone)
{
  ...
  rr.qtype=QType::A;
  rr.qname=DNSName("_underscore")+DNSName(zone);
  rr.content="127.0.0.1";
  db->feedRecord(rr, DNSName());

  rr.qname=DNSName("bla")+DNSName(zone);
  cout<<"Committing"<<endl;
  db->commitTransaction();
...
static const std::unordered_map<std::string, commandDispatcher> commands{
  ...
  {"network-set", {true, networkSet, GROUP_VIEWS,
   ...
   "\tSet the view for a network. Pass '' for the view to delete."}},
