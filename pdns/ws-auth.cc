...
{
  string zoneId = apiZoneNameToId(domainInfo.zone.operator const DNSName&());
...
public:
  TSIGKeyData(HttpRequest* req) :
    keyName(DNSName(apiZoneIdToName(req->parameters["id"])))
  {
    try {
...
static void apiServerZonesPOST(HttpRequest* req, HttpResponse* resp)
{
  ...
  for (auto& resourceRecord : new_records) {
    ...
    if (!resourceRecord.qname.isPartOf(zonename) && resourceRecord.qname != DNSName(zonename)) {
    ...
    if (resourceRecord.qtype.getCode() == QType::SOA && resourceRecord.qname == DNSName(zonename)) {
      have_soa = true;
    }
    if (resourceRecord.qtype.getCode() == QType::NS && resourceRecord.qname == DNSName(zonename)) {
      have_zone_ns = true;
    }
  }
  ...
  updateDomainSettingsFromDocument(backend, domainInfo, ZoneName(zonename), document, !new_records.empty());
  ...
  fillZone(backend, ZoneName(zonename), resp, req);
  resp->status = 201;
}
...
static void apiServerZonesGET(HttpRequest* req, HttpResponse* resp)
{
  ...
  if (req->getvars.count("zone") != 0) {
    ...
    ZoneName zonename = ZoneName(apiNameToDNSName(zone));
    zonename.makeUsLowerCase();
    DomainInfo domainInfo;
    if (backend.getDomainInfo(zonename, domainInfo)) {
...
static void apiServerZoneDetailPUT(HttpRequest* req, HttpResponse* resp)
{
  ...
  if (rrsets.is_array()) {
    ...
    for (auto& resourceRecord : new_records) {
      resourceRecord.qname.makeUsLowerCase();
      if (!resourceRecord.qname.isPartOf(zoneData.zoneName) && resourceRecord.qname != DNSName(zoneData.zoneName)) {
        throw ApiException("RRset " + resourceRecord.qname.toString() + " IN " + resourceRecord.qtype.toString() + ": Name is out of zone");
      }
      apiCheckQNameAllowedCharacters(resourceRecord.qname.toString());

      if (resourceRecord.qtype.getCode() == QType::SOA && resourceRecord.qname == DNSName(zoneData.zoneName)) {
        haveSoa = true;
      }
    }
...
static void patchZone(UeberBackend& backend, const ZoneName& zonename, DomainInfo& domainInfo, HttpRequest* req, HttpResponse* resp)
{
  ...
  try {
    ...
    for (const auto& rrset : rrsets.array_items()) {
      ...
      ZoneName qname = ZoneName(apiNameToDNSName(stringFromJson(rrset, "name")));
      apiCheckQNameAllowedCharacters(qname.toString());
      QType qtype;
      qtype = stringFromJson(rrset, "type");
      if (qtype.getCode() == 0) {
      ...
      if (changetype == "DELETE") {
        ...
        if (!domainInfo.backend->replaceRRSet(domainInfo.id, DNSName(qname), qtype, vector<DNSResourceRecord>())) {
          throw ApiException("Hosting backend does not support editing records.");
        }
      }
        ...
        try {
          if (replace_records) {
            ...
            uint32_t ttl = uintFromJson(rrset, "ttl");
            gatherRecords(rrset, qname.operator DNSName&(), qtype, ttl, new_records);
        ...
        if (replace_records) {
          ...
          domainInfo.backend->lookup(QType(QType::ANY), qname.operator const DNSName&(), static_cast<int>(domainInfo.id));
          DNSResourceRecord resourceRecord;
          while (domainInfo.backend->get(resourceRecord)) {
          ...
          if (!new_records.empty() && ent_present) {
            QType qt_ent{0};
            if (!domainInfo.backend->replaceRRSet(domainInfo.id, DNSName(qname), qt_ent, new_records)) {
              throw ApiException("Hosting backend does not support editing records.");
            }
          }
          if (!domainInfo.backend->replaceRRSet(domainInfo.id, DNSName(qname), qtype, new_records)) {
            throw ApiException("Hosting backend does not support editing records.");
          }
        }
        if (replace_comments) {
  ...
  for (const DomainInfo& domainInfo : domains) {
    if ((objectType == ObjectType::ALL || objectType == ObjectType::ZONE) && ents < maxEnts && simpleMatch.match(domainInfo.zone)) {
      doc.push_back(Json::object{
        {"object_type", "zone"},
        {"zone_id", apiZoneNameToId(domainInfo.zone.operator const DNSName&())},
        {"name", domainInfo.zone.toString()}});
      ents++;
    }
  ...
  if ((objectType == ObjectType::ALL || objectType == ObjectType::RECORD) && backend.searchRecords(qVar, maxEnts, result_rr)) {
    for (const DNSResourceRecord& resourceRecord : result_rr) {
      ...
      if (val != zoneIdZone.end()) {
        object["zone_id"] = apiZoneNameToId(val->second.zone.operator const DNSName&());
...
{
  ZoneName canon = ZoneName(apiNameToDNSName(req->getvars["domain"]));
